...
async function handlePostComment(
  ...
  try {
    // Parse request body (in camelCase)
    let data;
    try {
    ...
    // Validate inputs
    const validation = validateComment(snakeCaseData);
    if (!validation.valid) {
      return api.error(validation.error || 'Invalid comment data');
    }
    
    // Extract validated data
    const { sender_name, sender_email, content, captcha_token } = snakeCaseData;
    ...
    try {
      ...
      if (totalComments >= 64) {
        ...
        const batchResult = await db.batch([
          ...
          ).bind(postId, sender_name, sender_email || null, trimmedContent, clientIp)
        ]);
        ...
        const insertResult = await db.prepare(
          'INSERT INTO comments (post_id, sender_name, sender_email, content, ip) VALUES (?, ?, ?, ?, ?) RETURNING id, sender_name, sender_email, content, created_at'
        ).bind(postId, sender_name, sender_email || null, trimmedContent, clientIp).first<CommentModel>();
      ...
      return api.serverError('We encountered an issue saving your comment. Please try again later.');
    }
    
  } catch (error) {
    console.error('Error processing comment submission:', error);
    return api.serverError('An unexpected error occurred. Please try again later.');
  }
}
...
function validateComment(data): { valid: boolean; error?: string } {
  ...
  // Validate name
  if (!data.sender_name || typeof data.sender_name !== 'string' || data.sender_name.trim().length === 0) {
  ...
  if (data.content.length > 560) {
    return { valid: false, error: 'Comment cannot exceed 560 characters' };
  }
